...
def select_detection_input(tool_name: str, params: dict, input_data: dict):
    """Select the best available input and always keep raw TIFF as fallback."""
    ...
    for result in previous_results.values():
        for item in result.get("all_corrected_results", []) or []:
            item_payload = str(item.get("payload_type", payload_type)).lower()
            if (
                item_payload == payload_type
                and item.get("quality_accepted", True)
    ...
    if input_data.get("tiff_path"):
        candidates["raw"].append(input_data["tiff_path"])
    ...
    for source in orders.get(preference, orders["geo"]):
...
def call_specific_algorithm_docker(tool_name: str, target_name: str, params: dict, input_data: dict) -> dict:
    ...
    if not tiff_path:
        error_msg = f"目标检测阻断：未找到当前载荷({tool_name})可用的原图、预处理图或校正图"
